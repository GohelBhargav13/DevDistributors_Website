    ...
    if (isset($_SESSION['user_username']) && isset($_POST['buy-btn'])) {
        $user_id = $_SESSION['user_username'];
        $quantity = $_POST['quantity'];
        $product_id = $_SESSION['category_id'];
        $price = ($_POST['price']);

        intval($sub_price = ($price * $quantity));
        try {
            $sql = "INSERT INTO cart_item (user_id,product_id,quantity,price) VALUES (?,?,?,?) ";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("iiii", $user_id, $product_id, $quantity, $sub_price);

            if ($stmt->execute()) {
                $url = "product1.php?unit_size=". $product_details['unit_size'] . "&cat_id=" . $product_id;
                echo "<script>alert(`Item Added to Cart`);window.location.href='$url'</script>";
            } else {
                echo "<script>alert(`Something went Wrong...`)</script>";
            }
        } catch (Exception $th) {
            echo "<script>alert(`Here is error: $th`)</script>";
        }
    }
            ...
            <div class="row mt-5">
                ...
                <div class="col-lg-7 col-md-11 col-12 responsive " id="text-card">
                    ...
                    <form action="#" method="post">
                        <input type="hidden" name="price" value="<?= htmlspecialchars($product_details['price']) ?>">
                        <select name="quantity" class="my-3">
                            <option>Box Qty:</option>
                            <?php for ($i = 0; $i < 12; $i++) { ?>
                                <option value="<?= $i + 1 ?>"><?= htmlspecialchars($i + 1) ?></option>
                            <?php } ?>
                        </select>

                        <button id="buy-btn" name="buy-btn">Buy Now</button>
                    </form>
                    <h4 class="mt-5 mb-2">
                        Product Details :-
                    </h4>
